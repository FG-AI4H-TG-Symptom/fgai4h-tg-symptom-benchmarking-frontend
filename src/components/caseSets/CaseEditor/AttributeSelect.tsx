...
type AttributeSelectProps = (ConceptSelectionProps & AttributeSelectFixedProps) | ArrayFormComponentProps;
...
const AttributeSelect: React.FC<AttributeSelectProps> = (props) => {
  ...
  const attributeProperties = berlinModelSchema.definitions[attributeId]?.properties;

  const possibleValueReferences = attributeProperties?.value?.oneOf || attributeProperties?.values?.items.oneOf || [];
  ...
  if (attributeId && enabled === 'fixedAttribute' in props) {
    if (attributeProperties.value) {
      valuesSection = <ValueSelect name="value" possibleValues={dereferencedPossibleValues} />;
    } else {
      valuesSection = <ValueMultiSelect name="values" possibleValues={dereferencedPossibleValues} />;
    }
  }
