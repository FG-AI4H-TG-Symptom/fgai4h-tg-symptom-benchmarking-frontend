...
const calculateStatistics = (population: Population) => {
  const histograms = biologicalSexes.map((sex) =>
    datalib.histogram(population.filter(({ biologicalSex }) => biologicalSex === sex).map(({ age }) => age)),
  );

  const binLabels = histograms[0].map(({ value }) => `${value}-${value + histograms[0].bins.step}`);
  const maxCountInBin = Math.max(...histograms.map((histogram) => histogram.map(({ count }) => count)).flat());
...
const PopulationPyramid: React.FC<PopulationPyramidProps> = ({ population }) => {
  const { histograms, binLabels, maxCountInBin } = useMemo(() => calculateStatistics(population), [population]);
